...
module.exports = function(src, target, opts, initLog) {
    ...
    var awaitingDiff = processorQueue(getAwaitingDiffProcessor(filter, target, src), {
        retryInterval: 0,
        name: 'diff'
    });
...
var getAwaitingDiffProcessor = function(filter, target, source) {
    var that = function(queue, itemProcessed, log, callback) {
        ...
        async.forEachSeries(Object.keys(queue), function(seq, cbk) {
            ...
            retryHTTP(source.get, log.wrap('retryHTTP'))(cnhid, {
                revs: true,
                rev: doc._rev
            }, utils.cb(cbk, function(doc) {
...
var getAwaitingGetProcessor = function(src) {
    var that = processor(function(seq, payload, state, logs, callback) {
        ...
        async.forEachSeries(missing, function(rev, cbk2) {
            ...
            retryHTTP(src.get, logs.wrap('retryHTTP'))(change.id, {
                revs: true,
                rev: rev,
                attachments: true
            }, utils.safe(cbk2, function(error, got) {
                if (state.cancelled) {
                    return;
                }
                if (error) {
...
var checkExists = function(target, id, rev, log, callback) {
    ...
    retryHTTP(target.get, log.wrap('retryHTTP'))(id, {
        rev: rev
    }, utils.safe(callback, function(error, doc) {
        if (error) {
            ...
            checkExists(target, rev._id, rev._rev, logs.wrap('checking exists'), utils.cb(cbk, function(exists) {
                if (exists === false) {
                    retryHTTP(target.bulkDocs, logs.wrap('retryHTTP'))({
                        ...
                        new_edits: false
                    }, utils.safe.catchSyncronousErrors(cbk, function(error, response) {
                        if (state.cancelled) {
                            return;
                        }
